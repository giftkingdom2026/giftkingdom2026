...
class Customer extends Model
{
    ...
    public function handleSocialLoginCallback($social)
    {
        ...
        if ($social == 'google') {
            ...
            if (count($existUser) > 0) {
                ...
                $customers_id2 = DB::table('customers')->insertGetId([
                    'user_id' => $customers_id,
                    'google_id' => $social_id,
                ]);
            }
    ...
    {
        if (!empty(auth()->guard('customer')->user()->id)) {
             $current_customer=auth()->guard('customer')->user()->id;
        }else{
        ...
        if (!empty($current_customer)) {
            ...
            $liked_customers_id =$current_customer;
            ...
            if (count($record) > 0) {
                ...
                if (!empty($current_customer)) {
                    $total_wishlist = DB::table('liked_products')
                        ...
                        ->where('liked_customers_id', '=', $current_customer)->count();
                }
    ...
    public function unlikeMyProduct($id)
    {
        ...
        if (!empty(auth()->guard('customer')->user()->id)) {
            ...
            $liked_customers_id=$request->current_customer;
    ...
    public function wishlist($request)
    {
        ...
        {
            $cookie_id=$_COOKIE['is_like'];
        }else{
            $cookie_id=0;
        }
        $data = array('page_number' => 0, 'type' => 'wishlist', 'limit' => $limit, 'categories_id' => '', 'search' => '', 'min_price' => '', 'max_price' => '','is_like_cookie'=>$cookie_id);
        
        $products = $productss->products($data);
        $result['products'] = $products;

        $attrs= array('page_number' => 0, 'type' => '','limit' => $limit, 'categories_id' => '', 'search' => '', 'min_price' => '', 'max_price' => '');
        $all_products= $productss->products($attrs);
        ...
        if (count($user_email) > 0) {
            ...
            if (DB::table('users')->insert($data)
                ...
                if (auth()->guard('customer')->attempt(['email' => $request->email, 'password' => $request->password])) {
                    ...
                    $result['customers'] = $customers[0];

                    // $headers = "MIME-Version: 1.0" . "\r\n"; 
                    // $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n"; 
                    // mail($result['customers']->email
                    //     ,'Registration Notification',view("mail.createAccount")->with('data', $result['customers']),$headers);
