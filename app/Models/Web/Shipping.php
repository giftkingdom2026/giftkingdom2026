    ...
    public function addMyAddress($request)
{
    ...
    if (!empty($customers_id)) {
        ...
        DB::table('user_to_address')
            ->insert(['user_id' => $customers_id, 'address_book_id' => $address_book_id, 'is_default' => $is_default]);
    }
}
    ...
    public function getShippingAddress($address_id)
    {
        $addresses = DB::table('user_to_address')
            ...
            ->leftJoin('city', 'city.id', '=', 'address_book.entry_city')
        ...
        $result = $addresses->orderBy('default_address' , 'DESC')->get();
    ...
    {
        $allCountries = DB::table('countries')->where('status','1')->get();
        return ($allCountries);
