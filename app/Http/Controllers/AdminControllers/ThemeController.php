...
class ThemeController extends Controller
{
    ...
    public function Deal(Request $request)
    {
        ...
        $theme = new Theme();
        $result['offer'] = $theme->Deal();
        $result['commonContent'] = $this->Setting->commonContent();
        $images = new Images();
        $allimage = $images->getimages();
        // $result['images'] = $allimage;
        return view("admin.theme.deal", $title)->with('result', $result)->with('allimage',$allimage);
    }

    public function updateDeal(Request $request)
    {
        $theme = new Theme();
        $result = $theme->updateDeal($request);
        $message = Lang::get("Deal has been updated successfully");
        return redirect()->back()->withErrors([$message]);
